...
\`videoPrompt\` 仍然是一个字符串，但字符串内部必须写成“分镜式影视视频提示词”，不要使用【资产参考】【分镜提示词】【台词约束】【全局美术风格】这类旧标题。必须使用以下段落符号和语法：

1. **普通镜头段落**
   - 每个普通镜头段落必须以 \`·\` 开头，格式为：\`·【时代/景别/场景】主体动作 + 空间环境 + 氛围情绪。\`
   - 第一段必须包含本镜头必须保持一致的角色、场景、道具资产名称和当前状态；可以写入“@图1 作为……”这类参考图绑定，但不要凭空编造不存在的资产。
   - 角色参考必须包含：姓名/身份、服装、外貌状态、核心情绪；场景参考必须包含：时间、地点、光线、环境元素、气氛；道具参考必须包含：颜色、材质、磨损/血迹/反光、是否被角色拿起或特写。
   - 镜头必须承接上一镜最终状态，并在结尾留下可被下一镜接住的画面状态。

2. **强调镜头段落**
   - 需要局部特写、声音设计、动作转场或视觉重点时，使用 \`▲ 特写：\`、\`▲ 近景：\`、\`▲ 空镜：\` 等格式。
   - 强调镜头必须写清视觉焦点、局部动作、声音、转场方式和衔接对象，例如脚步声渐近、门缝光线扫过、镜头渐隐淡入到人物表情。

3. **台词与内心独白**
   - 如果 \`dialogue\` 有内容，必须把对白原文完整写入，并统一使用：\`【台词】【角色名】：“对白内容。”\`
   - 每个镜头只承载当前 \`dialogue\` 字段里的台词；不得新增、删减、改写、翻译台词。
   - 对白后的衔接可写“人声结束后，进入下面的镜头”。
   - 如果剧本中明确是旁白、心理活动或内心独白，使用：\`同步内心独白【角色名】：独白内容。\`
   - 若当前镜头没有台词，不能编造台词；只写环境声、呼吸声、动作声等画面声音。
   - 台词和内心独白只是语音指令，不代表画面文字，不得生成字幕或任何屏幕文字。

4. **动作、风格与禁止项**
   - 动作必须拆开写清：起始姿态、先动部位、发力方式、重心/视线/手脚变化、衣物或环境反馈、结束姿态。
   - 必须写清本项目统一画面风格、时代/地域质感、色调、光影、材质和镜头质感，但要自然并入镜头段落，不要单独使用旧标题。
   - 结尾必须单独写固定负面约束：\`不要生成字幕，不要生成任何屏幕文字、标题、标语、下三分之一字幕或对白字幕。\`
   - 同时保持人物外貌、服装、年龄、发型、道具和场景一致；写实电影质感、4K 高清、细节丰富、动作自然流畅；面部稳定不变形、五官清晰、人体结构正常、无卡顿、无闪烁、无 logo、无水印。

\`videoPrompt\` 格式示例：
·【现代近景/狭窄室内】@图1 作为 角色A 角色参考（身份、外貌、服装、压抑状态），@图2 作为 场景B 参考（夜晚、冷白顶灯、潮湿墙面、紧张空气）；角色A承接上一镜低头沉默的状态，肩线僵硬，手指贴着衣缝微微收紧，视线落在画面右侧的阴影里。
【台词】【角色A】：“原文台词。”人声结束后，进入下面的镜头

▲ 特写：镜头从角色A颤抖的手指开始，听见布料被捏紧的细微摩擦声，然后缓慢上移到下颌线；她先吞咽一下，再抬起下巴，眼眶被侧逆光切亮，渐隐淡入到眼部特写。

·【现代特写/同一室内】角色A眼神压住情绪，呼吸短促，衣袖被穿堂风轻轻带动；镜头固定在她含泪但没有眨动的眼睛上，结束时背景脚步声停在门外。
同步内心独白【角色A】：原文独白内容。
...
2. \`transition.incomingAction\` 和 \`videoPrompt\` 的第一个 \`·\` 镜头段落必须明确写出上一镜结束状态如何进入本镜；第一镜则写清冷开场起点。
...
5. 如果发生时间跳转、回溯、梦境或插叙，必须在 \`transition.timeGap\`、\`timeline\` 与 \`videoPrompt\` 的镜头段落中明示，不能让镜头关系含混。
`.trim();
...
9. **字段级动作要求**：
   ...
   - \`videoPrompt\`：必须是可直接给视频模型使用的**中文**成片级提示词，写成导演在调度运动镜头，而不是关键词堆砌；必须把“转身、离开、靠近、回头、对视、停顿、崩溃”等概括动作拆成连续可见动态；内部必须严格使用 \`·\` 普通镜头段落、\`▲\` 强调镜头段落、\`【台词】【角色名】：“...”\` 或 \`同步内心独白【角色名】：...\` 的分镜式影视视频提示词格式。
  ...
  "shots": [
    {
      ...
      "videoPrompt": "·【时代/景别/场景】@图1 作为 本镜头可见角色参考（身份、服装、外貌状态、情绪），@图2 作为 场景参考（时间、地点、光线、环境元素、气氛）；承接上一镜最终状态，写清主体起始姿态、镜头路径、动作拆解、环境反馈和结束画面。【台词】【角色名】：“当前 dialogue 字段原文台词。”人声结束后，进入下面的镜头\\n\\n▲ 特写：写清局部视觉焦点、动作声音、转场方式和衔接对象。\\n\\n·【时代/特写/同一场景】写清人物表情、视线、呼吸、衣物或环境反馈，以及留给下一镜承接的状态。若剧本明确为心理活动，使用 同步内心独白【角色名】：原文独白内容。\\n不要生成字幕，不要生成任何屏幕文字、标题、标语、下三分之一字幕或对白字幕。",
      "suggestedAssetNames": ["角色名", "场景名"],
      "characters": [
        {
          "name": "角色名",
...
1. \`videoPrompt\` 必须严格使用“分镜式影视视频提示词”格式，段落以 \`·\` 和 \`▲\` 组织；禁止使用【资产参考】【分镜提示词】【台词约束】【全局美术风格】四段旧格式。
2. 第一条 \`·\` 镜头段落必须绑定当前镜头可见角色、场景、道具和状态；可以使用“@图1 作为...”句式，但不要编造 @<<<image_n>>> 编号，最终视频提交时系统会自动补齐真实编号。
3. 每条 \`·\` 段落必须写成“【时代/景别/场景】主体动作 + 空间环境 + 氛围情绪”，只写当前镜头，不展开前后镜头；开头承接上一镜最终结果，结尾留下可被下一镜接住的画面状态。
4. \`▲\` 段落用于局部特写、声音设计、动作转场或视觉重点，必须写清视觉焦点、声音、转场方式和衔接对象。
5. 当前镜头规划里的 \`dialogue\` 原文必须完整进入 \`videoPrompt\`。对白使用 \`【台词】【角色名】：“对白内容。”\`；旁白、心理活动或内心独白使用 \`同步内心独白【角色名】：独白内容。\`；对白语言严格按剧本原文保留，不翻译、不改写。
6. 无台词时不能编造台词，只写环境声、呼吸声和动作声；结尾必须单独写“不要生成字幕，不要生成任何屏幕文字、标题、标语、下三分之一字幕或对白字幕。”
7. 镜头段落内必须自然包含项目风格、统一画面质感和禁止项：写实电影质感、4K 高清、动作自然流畅、面部稳定不变形、五官清晰、人体结构正常；禁止 logo、水印、风格漂移、现代元素错入。
...
## Output Format
{
  ...
  "videoPrompt": "·【时代/景别/场景】@图1 作为 本镜头可见角色参考（身份、服装、状态、情绪），@图2 作为 场景参考（时间、地点、光线、环境元素、气氛）；承接上一镜最终状态，写清人物动作、表情、环境反馈和结束画面。\\n【台词】【角色名】：“当前镜头对白原文。”人声结束后，进入下面的镜头\\n\\n▲ 特写：局部视觉焦点、动作声音、转场方式和衔接对象。\\n\\n·【时代/特写/同一场景】人物表情、视线、呼吸、衣物或环境反馈，停在可被下一镜承接的视觉状态。\\n不要生成字幕，不要生成任何屏幕文字、标题、标语、下三分之一字幕或对白字幕。",
  "suggestedAssetNames": ["角色名", "场景名"],
  "characters": [
    {
      "name": "角色名",
