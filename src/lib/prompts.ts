...
1. **【资产参考】**
   - 列出本镜头必须保持一致的角色、场景、道具资产名称和当前状态。
   - 不要凭空编造不存在的资产；优先使用 Existing Assets Context 或镜头规划中的资产名称。
   - 资产图编号由系统在最终提交视频时补齐；本字段重点写清“谁/什么/哪里必须一致”。

2. **【分镜提示词】**
   - 使用“镜头景别/镜头运动 + 画面主体 + 动作情绪 + 视觉细节 + 对应台词/画外音”的写法。
   - 必须写成本镜头可直接给视频模型执行的一条成片级分镜，不要写成关键词堆砌。
   - 必须包含：起始画面、镜头路径、主体动作拆解、环境反馈、结束画面/情绪落点。
   - 动作必须拆开写清：起始姿态、先动部位、发力方式、重心/视线/手脚变化、衣物或环境反馈、结束姿态。

3. **【台词约束】**
   - 如果 \`dialogue\` 有内容，必须把对白原文完整写入，格式为：同步画外音【角色名】：“台词” 或 同步对白【角色名】：“台词”。
   - 每个镜头只承载当前 \`dialogue\` 字段里的台词；不得新增、删减、改写台词。
   - 如果没有台词，明确写“无台词，只保留环境声/呼吸声/动作声”。
   - 人物口型不强制同步时，优先写“画外音”；需要角色开口时，才写“同步对白”。

4. **【全局美术风格】**
   - 写清本项目统一画面风格、时代/地域质感、色调、光影、材质、镜头质感和禁止项。
   - 必须结合项目风格，不要泛泛写“高级电影感”。
   - 必须加入一致性与负面限制：保持人物外貌、服装、年龄、发型、道具和场景一致；禁止与项目风格冲突的现代元素、媒介风格漂移、脸部变形、字幕和屏幕文字。

\`videoPrompt\` 格式必须使用这 4 个中文小标题，并写成可直接给视频模型使用的提示词，例如：
【资产参考】
角色A：保持已有角色资产的外貌、发型、服装和当前伤势状态；场景B：保持已有场景资产的空间结构与光线方向。

【分镜提示词】
（近景）镜头从角色A颤抖的手开始，缓慢上移到他的脸；他先收紧手指，再抬起下巴，视线越过画面右侧，衣袖被风吹动，最后停在含泪的眼部特写。

【台词约束】
同步画外音【角色A】：“原文台词。”

【全局美术风格】
写实电影感，低饱和色调，真实皮肤和布料质感，统一项目美术风格；禁止字幕、屏幕文字、现代元素、风格漂移和脸部变形。
`.trim();
  ...
  "shots": [
    {
      ...
      "videoPrompt": "【资产参考】列出本镜头可见角色、场景、道具资产名称和当前状态，优先使用已有资产名称，系统会在最终提交视频时补齐 @<<<image_n>>> 参考图编号。【分镜提示词】按“镜头景别/镜头运动 + 画面主体 + 动作情绪 + 视觉细节 + 对应台词/画外音”书写；明确起始画面、镜头路径、主体动作拆解、环境反馈和结束画面；禁止只写转身离开、走过去、看向某人这类概括动作。【台词约束】严格写入当前 dialogue 字段中的原文台词；每个镜头只承载当前标注台词，不新增、不删减、不改写；无台词时写明只保留环境声/呼吸声/动作声。【全局美术风格】结合项目风格写清统一画面质感、色调、光影、材质、镜头气质和禁止项；保持人物外貌、服装、年龄、发型、道具和场景一致；禁止字幕、屏幕文字、风格漂移和脸部变形。",
      "suggestedAssetNames": ["角色名", "场景名"],
      "characters": [
        {
          "name": "角色名",
...
2. 【分镜提示词】必须只写当前镜头，不要展开前后镜头；但要在开头承接上一镜最终结果，在结尾留下可被下一镜接住的画面状态。
3. 【台词约束】只能使用当前镜头规划里的 \`dialogue\` 原文；无台词时写“无台词，只保留环境声、呼吸声和动作声”。
4. 【资产参考】使用已有资产名称和 suggestedAssets 中的名称，不要编造 @<<<image_n>>> 编号，最终视频提交时系统会自动补齐真实编号。
5. 【全局美术风格】必须包含项目风格、统一画面质感和禁止项：禁止字幕、屏幕文字、风格漂移、脸部变形、现代元素错入。

## Output Format
{
  "sequence": ${shotPlan.sequence ?? 1},
  "duration": ${shotPlan.duration ?? DEFAULT_SHOT_DURATION_SECONDS},
  "dialogue": "当前镜头对白，使用 ${dialogueLanguageLabel}，没有则为空字符串",
  "videoPrompt": "【资产参考】本镜头可见资产名称与当前状态。【分镜提示词】当前镜头的景别/运镜 + 主体动作情绪 + 视觉细节 + 台词落点。【台词约束】当前镜头对白原文或无台词说明。【全局美术风格】项目统一画面风格、一致性要求和禁止项。",
  "suggestedAssetNames": ["角色名", "场景名"],
  "characters": [
    {
      "name": "角色名",
