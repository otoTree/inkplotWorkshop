...
11. **剧本片段覆盖**：所有 \`scriptExcerpt\` 合起来必须覆盖原剧本的关键动作、对白、转折和结尾钩子；冷开场可以前置高潮，但后续镜头必须回到原剧本顺序并补齐被前置前后的上下文。
...
7. **先写动作，再写解释**：每个镜头都优先写可见运动，再写情绪判断。重点交代身体力学、镜头路径、目光变化、物体位移、布料头发反馈、灰尘烟雾与光线变化。
8. **禁止弱镜头写法**：
   - 差例：\`低机位中景，人物悲伤抬头。\`
...
14. **资产选择必须精确克制**：\`suggestedAssets.characters\` 和 \`suggestedAssetNames\` 只能包含当前镜头实际可见的角色状态资产；同一角色只能保留一个最贴合当前状态的资产。若资产列表中同时存在通用角色图与 young/old/工作服/作战服/ep编号/场次编号等状态图，必须只选状态最准确的那一个。不要把同一角色的多张状态图、道具图、旧服装图一起输出。
