...
## videoPrompt 成片提示词结构（必须严格使用）
\`videoPrompt\` 仍然是一个字符串，但字符串内部必须按以下 4 个中文段落组织，顺序不可变，每段都要有明确内容，不能缺项，不能只写标题：

1. **【资产参考】**
   - 用一句连续描述绑定本镜头必须保持一致的角色、场景、道具资产名称和当前状态，写法参考：“@图1 作为 角色A 角色参考（身份、外貌、服装、状态、情绪），@图2 作为 场景B 参考（时间、地点、光线、环境元素、气氛）。”
   - 不要凭空编造不存在的资产；优先使用 Existing Assets Context 或镜头规划中的资产名称。
   - 资产图编号由系统在最终提交视频时补齐；本字段重点写清“谁/什么/哪里必须一致”。
   - 角色参考必须包含：姓名/身份、服装、外貌状态、核心情绪；场景参考必须包含：时间、地点、光线、环境元素、气氛；道具参考必须包含：颜色、材质、磨损/血迹/反光、是否被角色拿起或特写。

2. **【分镜提示词】**
   - 使用“时间段 + 镜头景别/镜头运动 + 画面主体 + 动作情绪 + 视觉细节 + 大括号对白”的写法。
   - 必须写成本镜头可直接给视频模型执行的一条成片级分镜，不要写成关键词堆砌。
   - 必须包含：起始画面、镜头路径、主体动作拆解、环境反馈、结束画面/情绪落点。
   - 动作必须拆开写清：起始姿态、先动部位、发力方式、重心/视线/手脚变化、衣物或环境反馈、结束姿态。
   - 若镜头时长超过 3 秒，按 0-3 秒、3-6 秒、6-9 秒这类时间段拆写；若镜头较短，也要写清起始、中段动作和结束落点。
   - 每个时间段只安排一个主要镜头目标，避免多人同时说话或复杂动作堆叠。
   - 第一段要承接上一镜最终结果；最后一段要用人物反应、关键动作、道具特写或环境变化收束，为下一镜留下可承接状态。

3. **【台词约束】**
   - 如果 \`dialogue\` 有内容，必须把对白原文完整写入，并统一使用大括号格式：\`{角色名: 对白内容}\`。
   - 对白语言严格遵循剧本原文，不擅自翻译；中文保持中文，英文保持英文，中英混合保持混合。
   - 每个镜头只承载当前 \`dialogue\` 字段里的台词；不得新增、删减、改写台词。
   - 如果没有台词，明确写“无台词，只保留环境声/呼吸声/动作声”。
   - 对白必须嵌入【分镜提示词】的动作句中，例如：“他侧过头直视对方，压低声音说出{角色A: 原文台词}。”
   - 对白中的角色名必须和当前动作中的说话人一致；如果原始信息冲突，按剧情逻辑修正说话人。
   - 对白只是语音指令，不代表画面文字，不得生成字幕或任何屏幕文字。

4. **【全局美术风格】**
   - 写清本项目统一画面风格、时代/地域质感、色调、光影、材质、镜头质感和禁止项。
   - 必须结合项目风格，不要泛泛写“高级电影感”。
   - 必须加入一致性与负面限制：保持人物外貌、服装、年龄、发型、道具和场景一致；写实电影质感、4K 高清、细节丰富、动作自然流畅；面部稳定不变形、五官清晰、人体结构正常、无卡顿、无闪烁；不要生成字幕、屏幕文字、标题、标语、下三分之一字幕、对白字幕、logo 或水印。

\`videoPrompt\` 格式必须使用这 4 个中文小标题，并写成可直接给视频模型使用的提示词，例如：
【资产参考】
@图1 作为 角色A 角色参考（身份、外貌、服装、当前伤势状态和压抑情绪），@图2 作为 场景B 参考（黄昏、狭窄室内、侧逆光、空气尘埃和紧张气氛），@图3 作为 道具C 参考（金属材质、磨损、血迹和冷光反射）。

【分镜提示词】
0-3 秒：近景手持推近，承接上一镜角色A低头沉默的状态。镜头从他颤抖的手开始，缓慢上移到他的脸；他先收紧手指，再抬起下巴，视线越过画面右侧，压低声音说出{角色A: 原文台词}，衣袖被风吹动，最后停在含泪的眼部特写。

【台词约束】
{角色A: 原文台词}

【全局美术风格】
写实电影质感，低饱和冷暖色调交织，真实皮肤、布料磨损、空气尘埃和金属反光；保持所有角色外貌、服装、道具和场景严格一致。4K 高清，面部稳定不变形、五官清晰、人体结构正常、动作自然流畅、无卡顿、无闪烁。不要生成字幕、屏幕文字、标题、标语、下三分之一字幕、对白字幕、logo 或水印。
`.trim();
...
2. \`transition.incomingAction\` 和 \`videoPrompt\` 的【分镜提示词】必须明确写出上一镜结束状态如何进入本镜；第一镜则写清冷开场起点。
...
5. 如果发生时间跳转、回溯、梦境或插叙，必须在 \`transition.timeGap\`、\`timeline\` 与 \`videoPrompt\` 的【分镜提示词】中明示，不能让镜头关系含混。
`.trim();
...
2. **动词优先于名词**：动作 > 场景 > 风格。
3. **语言硬性规则**：
   - 除对白原文外，JSON 输出中的所有字段、所有说明、所有示例措辞、所有 \`videoPrompt\` 镜头说明都必须使用**中文**。
   - \`dialogue\` 字段必须严格保留剧本原文语言：${dialogueLanguageLabel}。剧本对白是中文就保留中文，是英文就保留英文，中英混合就保留中英混合，不擅自翻译或改写。
   - **禁止把英文风格词、镜头说明、情绪、运镜、光效、场景名、角色描述、约束语或标签词原样输出到非对白内容中**；若引用了英文风格描述，必须理解后改写成中文。只有剧本对白原文允许保留英文或中英混合。
...
9. **字段级动作要求**：
   ...
   - \`videoPrompt\`：必须是可直接给视频模型使用的**中文**成片级提示词，写成导演在调度运动镜头，而不是关键词堆砌；必须把“转身、离开、靠近、回头、对视、停顿、崩溃”等概括动作拆成连续可见动态；内部必须严格包含四个小标题：【资产参考】【分镜提示词】【台词约束】【全局美术风格】。
  ...
  "shots": [
    {
      ...
      "dialogue": "角色名：台词内容（或 旁白：内容），该字段必须保留剧本原文语言：${dialogueLanguageLabel}",
      ...
      "videoPrompt": "【资产参考】用“@图1 作为...”结构绑定本镜头可见角色、场景、道具资产名称和当前状态，优先使用已有资产名称，系统会在最终提交视频时补齐 @<<<image_n>>> 参考图编号。【分镜提示词】按“时间段 + 镜头景别/镜头运动 + 画面主体 + 动作情绪 + 视觉细节 + {角色名: 对白内容}”书写；明确起始画面、镜头路径、主体动作拆解、环境反馈和结束画面；禁止只写转身离开、走过去、看向某人这类概括动作；超过 3 秒的镜头要按 0-3 秒、3-6 秒等时间段拆写。【台词约束】严格写入当前 dialogue 字段中的原文台词，格式为 {角色名: 对白内容}；每个镜头只承载当前标注台词，不新增、不删减、不改写、不翻译；无台词时写明只保留环境声/呼吸声/动作声。【全局美术风格】结合项目风格写清统一画面质感、色调、光影、材质、镜头气质和禁止项；保持人物外貌、服装、年龄、发型、道具和场景一致；禁止字幕、屏幕文字、标题、标语、下三分之一字幕、对白字幕、logo、水印、风格漂移和脸部变形。",
      "suggestedAssetNames": ["角色名", "场景名"],
      "characters": [
        {
          "name": "角色名",
...
export const getStoryboardSystemPrompt = (artStyle?: ArtStyleInput) => {
  const { strategy } = resolvePromptStrategy(artStyle);
  return `全局美术风格：${strategy.storyboard.styleDirective}`;
};
...
只产出 sequence=${shotPlan.sequence ?? 1} / ${totalShots || '未知'} 这一个镜头。
...
## 当前镜头 videoPrompt 强制规则
1. \`videoPrompt\` 必须严格使用【资产参考】【分镜提示词】【台词约束】【全局美术风格】四段格式。
2. 【资产参考】必须用“@图1 作为...”的连续句式绑定当前镜头可见角色、场景、道具和状态；不要编造 @<<<image_n>>> 编号，最终视频提交时系统会自动补齐真实编号。
3. 【分镜提示词】必须只写当前镜头，不要展开前后镜头；但要在开头承接上一镜最终结果，在结尾留下可被下一镜接住的画面状态。超过 3 秒的镜头按 0-3 秒、3-6 秒等时间段拆写，每个时间段只安排一个主要镜头目标。
4. 【分镜提示词】中的对白必须嵌入动作句，并使用 \`{角色名: 对白内容}\` 格式；不要使用“同步对白【角色名】：“台词””或台词列表。
5. 【台词约束】只能使用当前镜头规划里的 \`dialogue\` 原文，格式为 \`{角色名: 对白内容}\`；对白语言严格按剧本原文保留，不翻译、不改写；无台词时写“无台词，只保留环境声、呼吸声和动作声”。
6. 【全局美术风格】必须包含项目风格、统一画面质感和禁止项：写实电影质感、4K 高清、动作自然流畅、面部稳定不变形、五官清晰、人体结构正常；禁止字幕、屏幕文字、标题、标语、下三分之一字幕、对白字幕、logo、水印、风格漂移、现代元素错入。

## Output Format
{
  "sequence": ${shotPlan.sequence ?? 1},
  "duration": ${shotPlan.duration ?? DEFAULT_SHOT_DURATION_SECONDS},
  "dialogue": "当前镜头对白，保留剧本原文语言：${dialogueLanguageLabel}，没有则为空字符串",
  "videoPrompt": "【资产参考】@图1 作为 本镜头可见角色/场景/道具参考（身份、服装、状态、光线、材质等一致性信息）。【分镜提示词】0-3 秒：景别与运镜，承接上一镜最终状态，描述人物动作、表情、环境反馈，并在动作句中嵌入{角色名: 对白内容}；结尾停在可被下一镜承接的视觉状态。【台词约束】{角色名: 对白内容}；无台词时写无台词，只保留环境声、呼吸声和动作声。【全局美术风格】项目统一画面风格、低饱和电影质感、真实材质与光影、角色和场景一致性；禁止字幕、屏幕文字、标题、标语、下三分之一字幕、对白字幕、logo、水印、风格漂移和脸部变形。",
  "suggestedAssetNames": ["角色名", "场景名"],
  "characters": [
    {
      "name": "角色名",
