...
import { isInternationalSeedance2Model, normalizeProjectVideoSettings } from '@/lib/volcengine/video-compat';
...
export function AssetGallery({ projectId }: { projectId: string }) {
  ...
  const usesInternationalSeedance = isInternationalSeedance2Model(normalizedVideoSettings.model);
  ...
  const getVolcengineErrorMessage = (value: unknown): string | null => {
    ...
    if (Array.isArray(value)) return value.map(getVolcengineErrorMessage).filter(Boolean).join('；') || null;
    if (typeof value !== 'object') return String(value);
    const error = value as Record<string, unknown>;
    const message = error.Message || error.message || error.ErrorMessage || error.error_message || error.details || error.Detail;
  ...
  const formatVolcengineSyncNotice = (result: VolcengineSyncResult, action: VolcengineSyncAction): VolcengineSyncNotice => {
    ...
    const actionLabel = action === 'refresh-status' ? '刷新' : action === 'retry-processing' ? '重试提交' : action === 'force-resync' ? '完全重新同步' : '同步';
    const details = [total > 0 ? `${actionLabel} ${total} 个` : null, active > 0 ? `已可用 ${active}` : null, processing > 0 ? `处理中 ${processing}` : null, failed > 0 ? `失败 ${failed}` : null, errors > 0 ? `查询异常 ${errors}` : null, skipped > 0 ? `跳过 ${skipped}` : null].filter(Boolean);
    if (failed > 0 || errors > 0) return { type: 'error', message: `${actionLabel}完成，但有素材失败：${details.join('，')}。请查看下方素材卡片里的失败原因。` };
    if (processing > 0) return { type: 'warning', message: `${actionLabel}已提交：${details.join('，')}。素材仍在火山处理中，稍后可刷新状态。` };
    return { type: 'success', message: details.length > 0 ? `${actionLabel}完成：${details.join('，')}。` : `${actionLabel}完成。` };
  };

  const runVolcengineSyncBatches = async (action: VolcengineSyncAction): Promise<VolcengineSyncResult> => {
    ...
    while (true) {
      const response = await fetch('/api/assets/sync-volcengine', {
        ...
        body: JSON.stringify({ projectId, action, cursor, batchSize: DEFAULT_VOLCENGINE_ASSET_BATCH_SIZE }),
      });
      const data = await response.json().catch(() => ({})) as VolcengineSyncResult & { error?: string };
      if (!response.ok) throw new Error(data.region ? `${data.error || '素材库批次请求失败，请稍后重试'}（节点 ${data.region}）` : data.error || '素材库批次请求失败，请稍后重试');
      for (const key of ['synced', 'refreshed', 'active', 'processing', 'failed', 'errors', 'skipped'] as const) aggregate[key] = (aggregate[key] || 0) + (data[key] || 0);
      batchCount += 1;
      if (!data.hasMore) break;
      if (!data.nextCursor || data.nextCursor === cursor) throw new Error('素材库批次游标未推进，已停止以避免重复请求');
      cursor = data.nextCursor;
      const processed = aggregate.synced ?? aggregate.refreshed ?? 0;
      setVolcengineSyncNotice({ type: 'warning', message: `正在分批处理素材：已完成 ${processed} 个${typeof data.remaining === 'number' ? `，剩余 ${data.remaining} 个` : ''}（第 ${batchCount} 批）。` });
    }
    return aggregate;
  };
  ...
  const getAssetSyncClassName = (asset: Asset) => {
    if (asset.volcengineAssetStatus === 'Active' && asset.volcengineAssetId) return 'border-emerald-200 bg-emerald-50 text-emerald-700';
    if (asset.volcengineAssetStatus === 'Processing') return 'border-sky-200 bg-sky-50 text-sky-700';
    if (asset.volcengineAssetStatus === 'Failed') return 'border-red-200 bg-red-50 text-red-700';
  ...
  const handleRetryProcessingVolcengineAssets = async () => {
    ...
    if (!confirm('将重新提交所有“处理中”的火山素材。若旧任务稍后完成，素材库里可能出现重复素材，是否继续？')) return;
    setIsSyncingVolcengineAssets(true);
    try {
  ...
  const handleForceResyncVolcengineAssets = async () => {
    ...
    if (!confirm(`将清空 ${syncableAssets.length} 个资产的旧火山素材记录，并重新创建素材组后全部重传。旧素材不会从火山素材库删除，是否继续？`)) return;
    setIsSyncingVolcengineAssets(true);
    try {
  ...
  const activeAssetCount = syncableAssets.filter(a => a.volcengineAssetStatus === 'Active' && a.volcengineAssetId).length;
  const processingAssetCount = syncableAssets.filter(a => a.volcengineAssetStatus === 'Processing' && a.volcengineAssetId).length;
  ...
  const resyncCandidateCount = syncableAssets.filter(a => !a.volcengineAssetId || a.volcengineAssetStatus === 'Failed').length;
  const latestSyncTime = syncableAssets
    ...
    .sort((a, b) => b - a)[0];
  const latestSyncLabel = latestSyncTime ? formatSyncTime(new Date(latestSyncTime).toISOString()) : '暂无记录';
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className={volcengineSyncEnabled ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-50 text-slate-500'}>
                {usesInternationalSeedance ? '国际版不需要' : volcengineSyncEnabled ? '已开启' : '未开启'}
              </Badge>
            </div>
            <p className="mt-1 text-xs text-black/45">
              {usesInternationalSeedance ? '当前使用国际版模型，视频生成会直接使用对象存储链接。' : '状态来自资产库记录；视频生成前会把关联资产同步到火山素材库。'}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" size="sm" onClick={handleRefresh} disabled={isRefreshing || isSyncingVolcengineAssets} className="w-fit">
              <RefreshCw className={`mr-2 h-4 w-4 ${isRefreshing ? 'animate-spin' : ''}`} />刷新状态
            </Button>
            <Button type="button" size="sm" onClick={handleResyncVolcengineAssets} disabled={usesInternationalSeedance || !volcengineSyncEnabled || isSyncingVolcengineAssets || resyncCandidateCount === 0} className="w-fit bg-black text-white hover:bg-black/80" title={volcengineSyncEnabled ? '只同步未同步和失败的资产' : '请先在项目设置中开启素材库同步'}>
              <RefreshCw className={`mr-2 h-4 w-4 ${isSyncingVolcengineAssets ? 'animate-spin' : ''}`} />{isSyncingVolcengineAssets ? '同步中...' : `重新同步 (${resyncCandidateCount})`}
            </Button>
            {processingAssetCount > 0 && (
              <Button type="button" variant="outline" size="sm" onClick={handleRetryProcessingVolcengineAssets} disabled={usesInternationalSeedance || !volcengineSyncEnabled || isSyncingVolcengineAssets} className="w-fit border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100">
                <RefreshCw className={`mr-2 h-4 w-4 ${isSyncingVolcengineAssets ? 'animate-spin' : ''}`} />重试处理中 ({processingAssetCount})
              </Button>
            )}
            <Button type="button" variant="outline" size="sm" onClick={handleForceResyncVolcengineAssets} disabled={usesInternationalSeedance || !volcengineSyncEnabled || isSyncingVolcengineAssets || syncableAssets.length === 0} className="w-fit border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" title={volcengineSyncEnabled ? '清空当前同步记录，创建新素材组后全部重新提交' : '请先在项目设置中开启素材库同步'}>
              <RefreshCw className={`mr-2 h-4 w-4 ${isSyncingVolcengineAssets ? 'animate-spin' : ''}`} />完全重新同步 ({syncableAssets.length})
            </Button>
          </div>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-5">
          <div className="rounded-md bg-black/[0.03] px-3 py-2"><p className="text-[10px] uppercase tracking-widest text-black/35">可同步图片</p><p className="mt-1 text-lg font-semibold text-black/80">{syncableAssets.length}</p></div>
          <div className="rounded-md bg-emerald-50 px-3 py-2"><p className="text-[10px] uppercase tracking-widest text-emerald-600/65">可用于生成</p><p className="mt-1 text-lg font-semibold text-emerald-700">{activeAssetCount}/{syncableAssets.length}</p></div>
          <div className="rounded-md bg-sky-50 px-3 py-2"><p className="text-[10px] uppercase tracking-widest text-sky-600/65">处理中</p><p className="mt-1 text-lg font-semibold text-sky-700">{processingAssetCount}</p></div>
          <div className="rounded-md bg-red-50 px-3 py-2"><p className="text-[10px] uppercase tracking-widest text-red-600/65">失败</p><p className="mt-1 text-lg font-semibold text-red-700">{failedAssetCount}</p></div>
          <div className="rounded-md bg-black/[0.03] px-3 py-2"><p className="text-[10px] uppercase tracking-widest text-black/35">最后同步</p><p className="mt-1 text-sm font-semibold text-black/75">{latestSyncLabel}</p></div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-black/45">
          <span>已提交到素材库 {submittedAssetCount} 张</span><span>未同步 {unsyncedAssetCount} 张</span><span>待重新同步 {resyncCandidateCount} 张</span>
          {normalizedVideoSettings.assetGroupId && <span>素材组 {normalizedVideoSettings.assetGroupId}</span>}
        </div>
        {volcengineSyncNotice && <div className={`mt-3 rounded-md border px-3 py-2 text-xs ${volcengineSyncNoticeClassName}`}>{volcengineSyncNotice.message}</div>}
      </div>
                        ...
                        <CardContent className="p-4 pt-0 text-xs text-black/50">
                            ...
                            {asset.imageUrl && (
                              <div className="mt-3 flex flex-wrap items-center gap-2">
                                <span className={`rounded border px-2 py-0.5 text-[10px] font-medium ${getAssetSyncClassName(asset)}`}>{getAssetSyncLabel(asset)}</span>
                                {asset.volcengineAssetId && <span className="max-w-full truncate rounded bg-black/[0.04] px-2 py-1 font-mono text-[10px] text-black/45">{asset.volcengineAssetId}</span>}
                                {asset.volcengineAssetSyncedAt && <span className="text-[10px] text-black/35">{formatSyncTime(asset.volcengineAssetSyncedAt)}</span>}
                              </div>
                            )}
                            {asset.imageUrl && volcengineErrorMessage && <p className="mt-2 line-clamp-3 rounded-md border border-red-100 bg-red-50 px-2 py-1 text-[11px] leading-5 text-red-700">失败原因：{volcengineErrorMessage}</p>}
                            {episodeLabel && (
